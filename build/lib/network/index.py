...
def as_PointCloudEdge(edge):
  if isinstance(edge, PointCloudEdge):
    return edge
  return PointCloudEdge(edge)
...
class LinearEdge(EdgeBase, SaveLoadMixin):
  """ Linear edge between two points. """

  def _lib(self):
    return {'p0': self.p0, 'p1': self.p1}

  def __init__(self, p0, p1):
    assert len(p0) == len(p1) == 2
    self.p0 = tuple(map(lambda x: rnd(float(x)), p0))
    self.p1 = tuple(map(lambda x: rnd(float(x)), p1))
    self._points = np.array([self.p0, self.p1])
  ...
  def split(self, i):
    assert 0 < i < 1
    p_12 = self._points.T @ np.array([1 - i, i])
    return LinearEdge(self.p0, p_12), LinearEdge(p_12, self.p1)

  @property
  def length(self):
    return ((self._points.T @ np.array([-1, 1])) ** 2).sum()


class PointCloudEdge(EdgeBase, SaveLoadMixin):
  ...
  def __init__(self, points, vertices=None, verts=None):
    if vertices is None:
    ...
    if self._verts is not None:
      self._verts = np.round(self._verts, EPSILON)
  ...
  def verts(self):
    if self._verts is None:
      self._verts = np.round(arclength_verts(self.points), EPSILON)
      assert valid_verts(self._verts)
    return self._verts
  ...
  @property
  def length(self):
    return np.round(np.sqrt(((self.points[1:] - self.points[:-1])**2).sum(1)).sum(), EPSILON)

  def is_arclength_parameterised(self, eps=1e-9):
    return (self.verts == np.round(arclength_verts(self.points), EPSILON)).all()
  ...
  def merge(self, other):
    ...
    return self.__class__(np.concatenate([self.points[:-1], other.points]), verts=verts)
  ...
  def copy(self):
    return self.__class__(self.points.copy(), verts=self._verts.copy())
  ...
  def flip(self):
    ...
    return self.__class__(self.points[::-1], verts=verts / verts[-1])
  ...
  def remove_duplicates(self, thresh=1e-13):
    ...
    return self.__class__(points[mask], verts=self.verts[mask])
  ...
  def linearize(self):
    return self.__class__(self.points[[0, -1]])
  ...
  def change_vertices(self, v0=None, v1=None):
    ...
    return self.__class__(self.inner_points, vertices=(v0, v1), verts=self._verts)
  ...
  def split(self, position):
    ...
    return self.__class__(p0, verts=v0), \
           self.__class__(p1, verts=v1)
  ...
  def refine(self, npoints=101, n=1, interpolargs=None):
    ...
    ret = self.__class__(points, verts=repfunc(xi))
...
class IndexSequence:
  ...
  def tuple_index(self, ind):
    """ same as tuple.index """
    return tuple(self).index(ind)
  ...
  def __repr__(self):
    return '{}({})'.format(self.__class__.__name__, list(self).__repr__())
  ...
  def __abs__(self):
    return self.__class__(list(map(abs, self.indices)), dtype=self.dtype)
...
def as_multiedge(arg):
  ...
  assert all( isinstance(_arg, PointCloudEdge) for _arg in arg )
...
class MultiEdge(IndexSequence):
  ...
  @staticmethod
  def new(*args, **kwargs):
    return as_multiedge(*args, **kwargs)

  def __init__(self, edges, dtype=None):
    super().__init__(edges, dtype=PointCloudEdge)
  ...
  def polygon(self):
    if not hasattr(self, '_polygon'):
      self._polygon = geometry.Polygon(self.points)
...
def as_Polygon(arg):
  if isinstance(arg, Polygon): return arg
  return Polygon(arg)


class Polygon(IndexSequence):

  def __init__(self, list_of_edges):
    super().__init__(list_of_edges, dtype=MultiEdge)

  @property
  def polygon(self):
    if not hasattr(self, '_polygon'):
      master, *slaves = map(lambda x: x.points, self)
      self._polygon = geometry.Polygon(master, holes=slaves or None)
    return self._polygon

  def check_vertices(self, thresh=1e-13):
    return all(map(lambda x: x.check_vertices(), self))


# Orientables


class OrientabilityMixin:

  @property
  def sign(self):
    if abs(self) == self: return 1
    return -1


class OrientableTuple(tuple, OrientabilityMixin):
...
class OrientableFloat(float, OrientabilityMixin):
  ...
  def __new__(cls, value, **kwargs):
    ret = super(OrientableFloat, cls).__new__(cls, round(value, EPSILON))
    assert 0 <= ret <= 1
    return ret

  def __init__(self, value, orientation=1):
  ...
  def __abs__(self):
    ...
    return OrientableFloat(1 - self, orientation=-1)

  def __mul__(self, other):
    assert other in (-1, 1), NotImplementedError
    if other == 1: return self
    return OrientableFloat(1 - self, orientation=-self.orientation)
...
class SplineEdge(EdgeBase, SaveLoadMixin):
  ...
  @lru_cache
  def as_PointCloudEdge(self, n=1000):
    xi = np.linspace(0, 1, n)
    evals = [ interpolate.splev(xi, (self.knotvector.extend_knots(),
                                     x, self.knotvector.degree)) for x in self.x.T ]
    return PointCloudEdge(np.stack(evals, axis=1), orientation=self.orientation)
