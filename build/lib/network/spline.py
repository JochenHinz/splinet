...
from network import EdgeNetwork, Network, plot, FrozenDict
...
from template import MultiPatchTemplate, FaceTemplate, singlepatch_template, \
                     double_diamond_template, diamond_template
from multipatch import MultiPatchBSplineGridObject, singlepatch, stack_MultiPatchBSplineGridObjects
...
def orthogonal_controlmaps(snetwork, orthogonalize_boundaries=False):
  if not all( len(template.patches) == 1 for template in snetwork.templates.values() ):
    raise AssertionError('Please apply all mappings first.')

  if not set(snetwork.maps.keys()) == set(snetwork.face_indices.keys()):
    raise AssertionError('Please set all maps first.')

  face_neighbours = snetwork.face_neighbours
  from orth import geometry_to_orthogonality

  index2side = dict(zip(range(4), ['bottom', 'right', 'top', 'left']))
  controlmaps = {}

  for face_index, G in snetwork.maps.items():
    myneighbours = face_neighbours[face_index]
    g, = G.break_apart()
    side_indices = [i for i in range(4) if myneighbours.get(i, face_index)[:2] != face_index[:2]]
    if orthogonalize_boundaries:
      side_indices = sorted(set(side_indices) | set(i for i in range(4) if i not in myneighbours))
    sides = [index2side[i] for i in side_indices]
    if sides:
      controlmaps[face_index] = (geometry_to_orthogonality(g, sides=sides), sides)
