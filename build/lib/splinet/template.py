...
from .aux import abs_tuple, _return_sorted_boundary_vertices, EdgeNotFoundError, default_ids
...
def _edge_neighbours(patches, edge):
  assert len(edge) == 2
...
def edge_neighbours(patches, edge):
  return _edge_neighbours(tuple(map(tuple, patches)), tuple(edge))
...
@lru_cache(maxsize=64)
def _ordered_boundary_edges_(temp):
...
def _ordered_boundary_edges(temp):
  """ By default the ordered boundary indices are ordered in such a way that the first edge edge0 = [i0, i1]
      is the one with the property that i0 equals the smallest index in the sequence. """
  patches = np.asarray(temp.patches, dtype=int)
  patchverts = np.asarray(temp.patchverts, dtype=float)
  slices = np.array([[0, 1], [2, 3], [0, 2], [1, 3]])
  all_edges = np.array(list(map(abs_tuple, patches[:, slices].reshape(-1, 2))))
  ...
  pol = Polygon(patchverts[boundary_vertex_indices])
  if pol != orient(pol):
    boundary_vertex_indices = np.roll(boundary_vertex_indices[::-1], 1)
  boundary_vertex_indices = np.concatenate([boundary_vertex_indices, boundary_vertex_indices[:1]])
  return (tuple(map(tuple, zip(boundary_vertex_indices, boundary_vertex_indices[1:]))),)


@lru_cache(maxsize=64)
def _to_networkx(patches):
  nverts = len(np.unique(np.asarray(patches, dtype=int)))
  G = nx.Graph()
  G.add_nodes_from(list(range(nverts)))
  all_edges = sorted(set(map(abs_tuple, chain(*[get_edges(patch) for patch in patches]))))
  G.add_edges_from(all_edges)
  return nx.freeze(G)


@lru_cache
def immediate_opposites(patches):
  """ In Nutils layout """
  immediate_opposites = defaultdict(set)
  for i, patch in enumerate(map(tuple, patches)):
    a, b, c, d = get_edges(patch)
    immediate_opposites[a].update({b})
    immediate_opposites[a[::-1]].update({b[::-1]})
    immediate_opposites[b].update({a})
    immediate_opposites[b[::-1]].update({a[::-1]})
    immediate_opposites[c].update({d})
    immediate_opposites[c[::-1]].update({d[::-1]})
    immediate_opposites[d].update({c})
    immediate_opposites[d[::-1]].update({c[::-1]})
  return {key: tuple(sorted(val)) for key, val in immediate_opposites.items()}


@lru_cache
def opposites(patches):
  """ In Nutils layout """
  opposites_old = {key: set(value) for key, value in immediate_opposites(patches).items()}
  while True:
  ...
  return {key: tuple(sorted(val)) for key, val in opposites.items()}


@lru_cache
def _find_knotvector_edges(patches) -> tuple:
  """ In Nutils layout """
  all_edges = set(map(abs_tuple, chain(*default_ids(patches))))
  imopp = immediate_opposites(patches)

  boundary_edges = set(map(abs_tuple, [key for key, val in imopp.items() if len(val) == 1]))
  opp = opposites(patches)
  ...
  while True:
    ...
    myopposites = set(map(lambda x: tuple(sorted(x)), opp[root]))
...
def find_knotvector_edges(patches) -> tuple:
  patches = tuple(map(tuple, patches))
  return _find_knotvector_edges(patches)


def refine_patch(patchverts, positions_x=None, positions_y=None):
  """ Patchverts in nutils ordering. """
  patchverts = np.asarray(patchverts)
  assert patchverts.shape == (4, 2)
  if positions_x is None and positions_y is None:
    return patchverts,
  positions_x = [0, *(positions_x or []), 1]
  positions_y = [0, *(positions_y or []), 1]
  positions_x, positions_y = map(lambda x: np.sort(np.unique(x)), (positions_x, positions_y))
  assert all(0 <= pos <= 1 for positions in (positions_x, positions_y) for pos in positions)
  a, b, c, d = patchverts[[0, 2, 3, 1]]

  patch = lambda x, y: a[_, _] * (1 - x[:, _, _]) * (1 - y[_, :, _]) + \
                       b[_, _] * (x[:, _, _]) * (1 - y[_, :, _]) + \
                       c[_, _] * (x[:, _, _]) * (y[_, :, _]) + \
                       d[_, _] * (1 - x[:, _, _]) * y[_, :, _]

  allpoints = patch(positions_x, positions_y)

  patches = []
  for i in range(allpoints.shape[0] - 1):
    for j in range(allpoints.shape[1] - 1):
      patches.append(np.stack([allpoints[i, j], allpoints[i, j+1], allpoints[i+1, j], allpoints[i+1, j+1]]))
...
class MultiPatchTemplate(SaveLoadMixin, OrderedBoundaryEdgesMixin):
  ...
  def __init__(self, patches, patchverts, knotvector_edges=None):
    assert all( len(patch) == 4 for patch in patches )
    ...
    if knotvector_edges is None:
      knotvector_edges = find_knotvector_edges(self.patches)
  ...
  def to_networkx(self):
    return _to_networkx(self.patches)
  ...
  def refine(self, edge, positions=.5):
    ...
    edge = tuple(edge)
    positions = sorted(map(lambda x: round(x, 5), positions))
    ...
    map_vert_index = dict(zip(self.patchverts, counter))
    ...
    for patch in self.patches:
      myverts = patchverts[list(patch)]
      edge_y, edge_x = patch[:2], patch[::2]
      newpatchverts.append( refine_patch(myverts, edge2position.get(edge_x, None), edge2position.get(edge_y, None)) )

    newpatchverts = np.round(np.concatenate(newpatchverts), 7)

    for vert in map(tuple, newpatchverts.reshape(-1, 2)):
    ...
    for patchverts in newpatchverts:
      ...
      for vert in patchverts:
        patch.append(map_vert_index[tuple(vert)])
      patches.append(patch)

    newpatchverts = tuple(sorted(map(tuple, np.unique(newpatchverts.reshape(-1, 2), axis=0)), key=lambda x: map_vert_index[x]))

    return self.__class__(patches, newpatchverts)
...
class FaceTemplate:
  ...
  def __init__(self, network, face_index, template, sides):
    ...
    self.template = self.template.__class__(**self.template._lib())
