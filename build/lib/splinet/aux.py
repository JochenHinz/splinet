...
def face_crosses(network, face_index):
  edges = list(network.get_edges(network.face_indices[face_index]))
  edges = edges[-1:] + edges
  ret = []
  for e0, e1 in zip(edges, edges[1:]):
    v0 = unit_vector(e0.points[-1] - e0.points[-2])
    v1 = unit_vector(e1.points[1] - e1.points[0])
    ret.append(np.cross(v0, v1))
  return np.asarray(ret)


def concave_corner_pattern(network, face_index, thresh):
  """ Return the pattern of the concave corners. Not strict. """
  return list(map(int, np.where(face_crosses(network, face_index) <= thresh)[0]))


def convex_corner_pattern(network, face_index, thresh):
  """ Return the pattern of the convex corners. Strict. """
  return list(map(int, np.where(face_crosses(network, face_index) > thresh)[0]))
...
def frozen(arr: np.ndarray, dtype=None) -> np.ndarray:
  if dtype is None:
    arr = np.asarray(arr)
  else:
...
@njit(cache=True)
def _return_sorted_boundary_vertices(edges):
  """ Given a list of boundary edges where edges[i][0] < edges[i][1],
      return the vertex indices in canonical ordering, starting from
  ...
  for i, edge in enumerate(edges):
    for a in edge:
      if a not in vertex2edge_index:
        vertex2edge_index[a] = List((i,))
      else:
        vertex2edge_index[a].append(i)

  ret = np.empty(edges.shape[:1], dtype=np.int64)
  ret[0] = prev_vertex = root
  prev_edge_index = vertex2edge_index[root][0]
  for i in range(1, len(ret)):
    prev_edge_index, = [a for a in vertex2edge_index[prev_vertex] if a != prev_edge_index]
    prev_vertex, = [a for a in edges[prev_edge_index] if a != prev_vertex]
    ret[i] = prev_vertex
  return ret
