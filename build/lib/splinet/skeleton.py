...
@lru_cache(maxsize=32)
def floater(Xi, X, shape0, shape1, ref=4, maxpoints=200):
  Xi = np.frombuffer(Xi, dtype=float).reshape(shape0)
  X = np.frombuffer(X, dtype=float).reshape(shape1)
  ...
  tri = Triangulation.from_polygon(X)
  vertices_geom = tri.points

  vertices_param = tri.floater(Xi, as_Triangulation=False)
...
def compute_vertex_positions(Xi, X, verts, ref=3, maxpoints=200):
  ...
  Xi, X = map(lambda x: np.asarray(x, dtype=float), (Xi, X))

  vertices_param, vertices_geom = floater(Xi.tobytes(), X.tobytes(), Xi.shape, X.shape, ref=ref, maxpoints=maxpoints)
...
def draw_skeleton(network: EdgeNetwork, ref=4,
                                        maxpoints=200,
                                        ax=None,
                                        show=True,
                                        fontsize=8,
