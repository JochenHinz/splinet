...
def univariate_fit(pointcloud, knotvector=None, btol=1e-3, mu=None, maxiter=8, constrain_tangent=False):
  ...
  for side, t in zip(('left', 'right'), (t0, t1)):
    ...
    if constrain_tangent:
      p = knotvector.degree
      if side == 'left':
        dx = knotvector.knots[1] - knotvector.knots[0]
        c0 = cons.reshape(-1, 2)[0]
        cons.reshape(-1, 2)[1] = dx / p * t + c0
      else:
        dx = knotvector.knots[-1] - knotvector.knots[-2]
        c1 = cons.reshape(-1, 2)[-1]
        cons.reshape(-1, 2)[-2] = c1 - dx / p * t
