...
import pygmsh
from mapping.sol import make_matrix as _make_matrix, MKL_from_scipy
...
def make_matrix(triangles, points, shape0, shape1):
  return MKL_from_scipy(_make_matrix(np.frombuffer(triangles, dtype=int).reshape(shape0),
                                     np.frombuffer(points, dtype=float).reshape(shape1)))


@lru_cache(maxsize=32)
def _make_mesh(X, shape, max_dist):
  X = np.frombuffer(X, dtype=float).reshape(shape)
  with pygmsh.geo.Geometry() as geom:
    geom.add_polygon(X, mesh_size=1.1*max_dist)  # make sure no additional boundary vertices are introduced
    tri = geom.generate_mesh(algorithm=5, dim=2, order=1)
  return tri


def make_mesh(X, max_dist):
  assert max_dist > 0
  return _make_mesh(X.astype(float).tobytes(), X.shape, max_dist)


@lru_cache(maxsize=32)
def floater(Xi, X, ref=4, maxpoints=200):
  Xi, X = map(np.asarray, (Xi, X))

  XiPc = pc.PointCloud(np.concatenate([Xi, Xi[:1]]))
  XPc = pc.PointCloud(np.concatenate([X, X[:1]]))

  xiverts = np.concatenate([XiPc.verts, [1]])
  xverts = np.concatenate([XPc.verts, [1]])
  for i in range(ref):
    xiverts = refine_verts(xiverts)
    xverts = refine_verts(xverts)
    if len(xiverts) > maxpoints:
      break

  Xi = XiPc.toInterpolatedUnivariateSpline(k=1)(xiverts)[:-1]
  X = XPc.toInterpolatedUnivariateSpline(k=1)(xverts)[:-1]

  max_dist = np.linalg.norm(X[1:] - X[:-1], axis=1).max()

  tri = make_mesh(X, max_dist)
  
  triangles = tri.cells_dict['triangle'].astype(int)
  points = tri.points.astype(float)
  M = make_matrix(triangles.tobytes(), points.tobytes(), triangles.shape, points.shape)
...
def compute_vertex_positions(Xi, X, verts, ref=3, maxpoints=200):

  vertices_param, vertices_geom = floater(tuple(map(tuple, Xi)), tuple(map(tuple, X)), ref=ref, maxpoints=maxpoints)
...
def draw_skeleton(network: EdgeNetwork, ref=4, maxpoints=200, ax=None, show=True, fontsize=8, templates_only=False, vertex_thickness=8, show_indices=True, face_colors=None):
  if templates_only:
  ...
  if face_colors is None:
    face_colors = subnetwork._colors
  ...
  for face, facetemplate in subnetwork.templates.items():
    ...
    myvertices = compute_vertex_positions(Xi, X, patchverts, ref=ref, maxpoints=maxpoints)
  ...
  try:
    ...
    ax.scatter(*Xblack.T, s=vertex_thickness, c='k')
  except Exception:
    pass

  Xred = np.stack([coords[i] for i in fixed_indices])
  ax.scatter(*Xred.T, s=int(1.5 * vertex_thickness), c='r')


  for face_index, indices in face_vertices.items():
    mypol = np.stack([coords[i] for i in indices])
    ax.fill(*mypol.T, color=face_colors[face_index[0]], alpha=.1)

    if show_indices:
      point = np.concatenate(ops.polylabel(geometry.Polygon(mypol)).coords.xy)
      ax.text(*point, str(list(face_index)), fontsize=fontsize, color=face_colors[face_index[0]], zorder=10)
